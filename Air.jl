module Air_PT2h
    ...
    data_PT = XLSX.readxlsx("Air_PT.xlsx")
    ...
    ######################## PT_h ########################
    data_coeffs = XLSX.readxlsx("Air_PT2H.xlsx")
    ...
    @sync @distributed for i=1:length(test_P)
        @distributed for j=1:length(test_T)
            ERR[i,j] = abs(PT_h(test_P[i],test_T[j]) - CP.PropsSI("H","P",test_P[i],"T",test_T[j],"Air"))/CP.PropsSI("H","P",test_P[i],"T",test_T[j],"Air")
        end
    end
    ERR_Max = findmax(ERR)
    ERR_Max_h = ERR_Max[1]
    ...
    ERR_Max_PT_h = [
        ...
        ERR_Max_P,
        ERR_Max_T,
        CP.PropsSI("H","P",ERR_Max_P,"T",ERR_Max_T,"Air")
        ]
...
module Air_PT2s
    ...
    ######################## PT_h ########################
    data_coeffs = XLSX.readxlsx("Air_PT2S.xlsx")
    coeffs = zeros(n_P,n_T-1,4)
    for i = 1:n_P
        coeffs[i,:,:] = data_coeffs[string("Sheet",string(i))][:]
    end
    ...
    @sync @distributed for i=1:length(test_P)
        @distributed for j=1:length(test_T)
            ERR[i,j] = abs(PT_s(test_P[i],test_T[j]) - CP.PropsSI("S","P",test_P[i],"T",test_T[j],"Air"))/CP.PropsSI("S","P",test_P[i],"T",test_T[j],"Air")
        end
    end
    ERR_Max = findmax(ERR)
    ERR_Max_h = ERR_Max[1]
    ...
    ERR_Max_PT_s = [
        ...
        ERR_Max_P,
        ERR_Max_T,
        CP.PropsSI("S","P",ERR_Max_P,"T",ERR_Max_T,"Air")
        ]
    #print("Max_informations:\n",ERR_Max_Series,"\n")
    ######################################################
    export 
...
module Air_Ph2T
    ...
    ## Reading PT of Spline
    data_PT = XLSX.readxlsx("Air_Ph.xlsx")
    P_data = (data_PT["Sheet1"][:])'
    T_data = (data_PT["Sheet2"][:])'
    n_P = length(P_data) #Number of P
    n_T = length(T_data) #Number of T
    ...
    ######################## PT_h ########################
    data_coeffs = XLSX.readxlsx("Air_Ph2T.xlsx")
    coeffs = zeros(n_P,n_T-1,4)
    for i = 1:n_P
        coeffs[i,:,:] = data_coeffs[string("Sheet",string(i))][:]
    end
    ...
    @sync @distributed for i=1:length(test_P)
        @distributed for j=1:length(test_T)
            ERR[i,j] = abs(Ph_T(test_P[i],test_T[j]) - CP.PropsSI("T","P",test_P[i],"H",test_T[j],"Air"))/CP.PropsSI("T","P",test_P[i],"H",test_T[j],"Air")
        end
    end
    ERR_Max = findmax(ERR)
    ERR_Max_h = ERR_Max[1]
    ...
    ERR_Max_Ph_T = [
        ...
        ERR_Max_P,
        ERR_Max_T,
        CP.PropsSI("T","P",ERR_Max_P,"H",ERR_Max_T,"Air")
        ]
    #print("Max_informations:\n",ERR_Max_Series,"\n")
    ######################################################
    export 
...
module Air_Ph2s
    ...
    ######################## PT_h ########################
    data_coeffs = XLSX.readxlsx("Air_Ph2s.xlsx")
    coeffs = zeros(n_P,n_T-1,4)
    for i = 1:n_P
        coeffs[i,:,:] = data_coeffs[string("Sheet",string(i))][:]
    end
    ...
    @sync @distributed for i=1:length(test_P)
        @distributed for j=1:length(test_T)
            ERR[i,j] = abs(Ph_s(test_P[i],test_T[j]) - CP.PropsSI("S","P",test_P[i],"H",test_T[j],"Air"))/CP.PropsSI("S","P",test_P[i],"H",test_T[j],"Air")
        end
    end
    ERR_Max = findmax(ERR)
    ERR_Max_h = ERR_Max[1]
    ...
    ERR_Max_Ph_s = [
        ...
        ERR_Max_P,
        ERR_Max_T,
        CP.PropsSI("S","P",ERR_Max_P,"H",ERR_Max_T,"Air")
        ]
    #print("Max_informations:\n",ERR_Max_Series,"\n")
    ######################################################
    export 
...
module Air_Ps2h
    ...
    ## Reading PT of Spline
    data_PT = XLSX.readxlsx("Air_Ps.xlsx")
    P_data = (data_PT["Sheet1"][:])'
    T_data = (data_PT["Sheet2"][:])'
    n_P = length(P_data) #Number of P
    n_T = length(T_data) #Number of T
    ...
    ######################## PT_h ########################
    data_coeffs = XLSX.readxlsx("Air_PS2H.xlsx")
    coeffs = zeros(n_P,n_T-1,4)
    for i = 1:n_P
        coeffs[i,:,:] = data_coeffs[string("Sheet",string(i))][:]
    end
    ...
    @sync @distributed for i=1:length(test_P)
        @distributed for j=1:length(test_T)
            ERR[i,j] = abs(Ps_h(test_P[i],test_T[j]) - CP.PropsSI("H","P",test_P[i],"S",test_T[j],"Air"))/CP.PropsSI("H","P",test_P[i],"S",test_T[j],"Air")
        end
    end
    ERR_Max = findmax(ERR)
    ERR_Max_h = ERR_Max[1]
    ...
    ERR_Max_Ps_h = [
        ...
        ERR_Max_P,
        ERR_Max_T,
        CP.PropsSI("H","P",ERR_Max_P,"S",ERR_Max_T,"Air")
        ]
    #print("Max_informations:\n",ERR_Max_Series,"\n")
    ######################################################
    export 
...
module Air_Ps2T
    ...
    ######################## PT_h ########################
    data_coeffs = XLSX.readxlsx("Air_PS2T.xlsx")
    coeffs = zeros(n_P,n_T-1,4)
    for i = 1:n_P
        coeffs[i,:,:] = data_coeffs[string("Sheet",string(i))][:]
    end
    ...
    @sync @distributed for i=1:length(test_P)
        @distributed for j=1:length(test_T)
            ERR[i,j] = abs(Ps_T(test_P[i],test_T[j]) - CP.PropsSI("T","P",test_P[i],"S",test_T[j],"Air"))/CP.PropsSI("T","P",test_P[i],"S",test_T[j],"Air")
        end
    end
    ERR_Max = findmax(ERR)
    ERR_Max_h = ERR_Max[1]
    ...
    ERR_Max_Ps_T = [
        ...
        ERR_Max_P,
        ERR_Max_T,
        CP.PropsSI("T","P",ERR_Max_P,"S",ERR_Max_T,"Air")
        ]
    #print("Max_informations:\n",ERR_Max_Series,"\n")
    ######################################################
    export 
